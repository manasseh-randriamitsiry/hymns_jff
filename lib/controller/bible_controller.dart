...
class BibleController extends GetxController {
  ...
  void _loadHighlights() {
    ...
    _highlightService.getHighlightsStream(selectedBook.value, selectedChapter.value).listen((highlightList) {
      highlights.value = highlightList;
    });
    
    // Load public highlights
    _highlightService.getPublicHighlightsStream(selectedBook.value, selectedChapter.value).listen((highlightList) {
  ...
  bool isVerseSelected(int verse) {
    return verse >= startVerse.value && verse <= endVerse.value && isSelecting.value || 
           (verse == startVerse.value && startVerse.value == endVerse.value && isSelecting.value);
  }
  ...
  Future<void> saveHighlight() async {
    ...
    if (success) {
      ...
      Get.snackbar('Fametrahana', 'Nisy olana teo am-pametrahana ny famaritaka.');
    }
  }
